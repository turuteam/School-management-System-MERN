...
route.post('/set/:id', async(req, res) => {
    //check if exist
   let isAdded = await CurrentModel.findOne({code: req.params.id, "years" : {"$in" : [req.body.currentYear]}})
    //create id

    if(!isAdded){
      await  CurrentModel.findOneAndUpdate({code: req.params.id}, {$push: {years: req.body.currentYear}})
    }
