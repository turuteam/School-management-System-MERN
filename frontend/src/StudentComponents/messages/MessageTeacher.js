...
function MessageTeacher() {
    ...
    const onSend = (e) => {
        ...
        if(message && recipient){
            setloading(true)
            axios
            .post(`/chats/send/user/${user?.id}/${recipient}`, {message, senderID: user?.id})
            .then((res) => {
                setloading(false)
                if(res.data.error){
      ...
      const searchOptions = () => {
        return recipientOptions.map(option => <option key={option.id} value={option.id}>{option.name} {option.surname} {option.id}  </option>)
     }
