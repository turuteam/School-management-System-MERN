...
function Dormitories() {
    ...
    const handleDelete = (id) => {
        setloading(true)
        axios.delete(`/dormitories/delete/${id}`).then(res => {
            setloading(false)
            if(res.data.error){
    ...
    const handleCreate = (e) => {
        ...
        axios.post('/dormitories/create', {name})
        .then(res => {
            ...
            setdormitories([...dormitories, res.data.docs]);
            setname("");
        })
        .catch(err => {
             setcreateLoading(false);
    ...
    const onEdit = (e) => {
        ...
        console.log("edited")
        setloading(true)
        axios.put(`/dormitories/update/${editID}`, {name: editname}).then(res => {
            setloading(false)
            console.log(res)
            if(res.data.error){
                errorAlert(res.data.error)
