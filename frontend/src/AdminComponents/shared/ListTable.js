...
export default function CustomPaginationActionsTable({data, tableHeader, handleEdit, handleDelete, loading, isCanteen ,isEdit,  noActions}) {
                  ...
                  {tableHeader && 
                    tableHeader?.map(cell =>   
                          <TableCell key={cell?.id}  align="left">
                              { isDate(row[cell?.id])  ? moment(row[cell?.id])?.format('D MMMM YYYY')   :   row[cell?.id] || "-"}
                          </TableCell>
                  )}
                  {!noActions &&
                    <TableCell  align="left" className="d-flex align-items-center">
                        <IconButton onClick={() => handleDelete(isCanteen? row.memberID : row._id)}>
                            <DeleteOutlineIcon/>
                        </IconButton>
                        {!isEdit &&
                        <IconButton onClick={() =>  handleEdit(isCanteen? row.memberID : row._id)}>
                          <EditIcon></EditIcon>
                        </IconButton>
                        }
                    </TableCell>
