...
function MemberPayments({member, planName}) {
    ...
    return (
        <div className=" content__container">
            <div style={{background: "#051f3e"}} className="d-flex flex-column align-items-center p-3 text-light mb-4">
                <Avatar  src={getImgSrc(user?.profileUrl)} style={{width: "100px" , height: "100px"}}/>
                <h3>{getCapitalize(user?.name)} {getIntial(user?.middlename)} {getCapitalize(user?.surname)}  - {user?.userID}</h3>

                <div><strong>{member?.role} </strong> </div>
                <div>Canteen Membership ID:  {member?.memberID}</div>
                <hr/>
                <div><strong>Payment Plan <span className="text-danger">{planName}</span> </strong>  </div>
            ...
            <table className="table table-bordered">
                ...
                <tbody>
                    {member?.payments && member?.payments.map(e =>   
                       <tr key={e._id}>
