...
function AddCanteenPayment() {
    ...
    const handleSelectuser = (id) =>{
          ...
          let price = paymentPlan.find(e => e.plan === selectedmember?.paymentMethod)
    ...
    const handleAdd = () => {
        setloading(true)
        axios.put(`/canteen/payment/${studentID}`,{ amount, covers: (amount / planPrice) })
        .then(res =>{
            setloading(false)
            if(res.data.error){
        ...
        <div className="mb-5 row">
            <div className="col-sm-6  content__container">
                  <div className="mb-5">
                    ...
                    <select onChange={e => handleSelectuser(e.target.value)} className="form-select">
                        <option defaultValue hidden>Choose...</option>
                       {members.length > 0 ? 
                          members.map(e => 
                           <option key={e.memberID} value={e.memberID}>{e.name} {e.memberID}</option>) 
